...
Same structure/preprocessing as the comparison scripts, so results
are directly comparable across scripts (same train/test split logic,
...
# ============================================================
# 8. Train + evaluate: XGBoost and Decision Tree, for BOTH targets
# ============================================================
results = []
...
print("\nTraining XGBoost (category)...")
xgb_cat, xgb_cat_preds = run_model(
    "XGBoost", XGBClassifier(n_estimators=100, random_state=42, eval_metric="mlogloss"),
    X_train, y_cat_train, X_test, y_cat_test, "Category"
)

print("Training Decision Tree (category)...")
dt_cat, dt_cat_preds = run_model(
    "Decision Tree", DecisionTreeClassifier(random_state=42),
    X_train, y_cat_train, X_test, y_cat_test, "Category"
)

print("Training XGBoost (priority)...")
xgb_pri, xgb_pri_preds = run_model(
    "XGBoost", XGBClassifier(n_estimators=100, random_state=42, eval_metric="mlogloss"),
    X_train, y_pri_train, X_test, y_pri_test, "Priority"
)

print("Training Decision Tree (priority)...")
dt_pri, dt_pri_preds = run_model(
    "Decision Tree", DecisionTreeClassifier(random_state=42),
    X_train, y_pri_train, X_test, y_pri_test, "Priority"
)
