"""
Model comparison: Linear SVM vs KNN
------------------------------------
...
This is COMPARISON code only — nothing gets saved to models/ yet.
Only once the team picks a final winning model (across everyone's
models, not just these two) do you export that one to a .pkl file.
"""
...
# drop high-cardinality identifier-like columns (emails, names, raw dates)
...
# ============================================================
# 8. Train + evaluate: Linear SVM and KNN, for BOTH targets
# ============================================================
results = []
...
print("\nTraining Linear SVM (category)...")
svm_cat, svm_cat_preds = run_model(
    "Linear SVM", LinearSVC(max_iter=2000), X_train, y_cat_train, X_test, y_cat_test, "Category"
)

print("Training KNN (category)...")
knn_cat, knn_cat_preds = run_model(
    "KNN", KNeighborsClassifier(n_neighbors=5), X_train, y_cat_train, X_test, y_cat_test, "Category"
)

print("Training Linear SVM (priority)...")
svm_pri, svm_pri_preds = run_model(
    "Linear SVM", LinearSVC(max_iter=2000), X_train, y_pri_train, X_test, y_pri_test, "Priority"
)

print("Training KNN (priority)...")
knn_pri, knn_pri_preds = run_model(
    "KNN", KNeighborsClassifier(n_neighbors=5), X_train, y_pri_train, X_test, y_pri_test, "Priority"
)
...
# ============================================================
# print(classification_report(y_cat_test, svm_cat_preds, target_names=le_category.classes_))